...
Run `pnpm audit --prod` (also enforced in verify CI) and review full `pnpm audit` separately. A production-only result does not clear development-tool vulnerabilities. Consumer release preparation must check distribution rights and fresh public registry pins; see docs/VERIFICATION_STATUS.md.
